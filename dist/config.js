...
export function parseConfig(path, text) {
    ...
    try {
        ...
        if (url.pathname !== "" && url.pathname !== "/") {
            throw new ConfigError(`"server" must not name a database (${JSON.stringify(url.pathname)}); "database" is the prefix dataless appends the branch to`);
        }
        const database = requireString(obj, "database");
        if (!IDENTIFIER.test(database)) {
            throw new ConfigError(`"database" must be lowercase letters, digits and underscores: ${JSON.stringify(database)}`);
        }
        const template = optionalString(obj, "template");
        if (template !== undefined && !IDENTIFIER.test(template)) {
            throw new ConfigError(`"template" must be lowercase letters, digits and underscores: ${JSON.stringify(template)}`);
        }
 ...
 */
export function assertServerAllowed(config, env) {
    const host = new URL(config.server).hostname;
    if (LOCAL_HOSTS.has(host))
        return;
    if (config.allowRemote && env["DATALESS_ALLOW_REMOTE"] === "1")
        return;
    throw new ConfigError(`refusing to manage databases on ${host}: it is not this machine. ` +
...
export function databaseUrl(config, database) {
