...
import { assertServerAllowed, ConfigError, databaseUrl, loadConfig, } from "./config.js";
import { branchExists, BranchError, currentBranch, databaseName, isDetached, } from "./branch.js";
import { DatabaseError, dropDatabase, listManaged, readMeta } from "./db.js";
import { childEnv, openServer, provision, ProvisionError, reset, } from "./provision.js";
import { explain, selectPrunable } from "./prune.js";
...
function context(branchOverride) {
...
function protectedNames(config) {
    const names = new Set([config.database, "postgres", "template0", "template1"]);
    if (config.template)
        names.add(config.template);
    return names;
}
async function withServer(config, body) {
...
async function cmdRun(argv) {
    ...
    const provisioned = await withServer(config, (client) => provision(client, config, database, branch, log));
...
async function cmdList() {
    ...
    for (const entry of entries) {
        ...
        console.log(`${here} ${entry.database.padEnd(width)}  ${formatSize(entry.sizeBytes).padStart(9)}  ${busy.padEnd(7)}  ${entry.meta.lastUsedAt.slice(0, 19)}  ${entry.meta.branch}`);
    }
    return 0;
}
async function cmdPrune(argv) {
    const apply = argv.includes("--yes");
    const olderThan = Number(argv.find((arg) => arg.startsWith("--older-than"))?.split("=")[1] ?? 14);
    ...
    return await withServer(config, async (client) => {
        ...
        if (!apply)
            log(`${candidates.length} to drop; pass --yes to do it`);
        const busy = entries.filter((entry) => entry.connections > 0).length;
        if (busy > 0)
            log(`${busy} database(s) skipped because something is connected`);
        return 0;
    });
}
async function cmdDrop(argv) {
