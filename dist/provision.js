...
import { connectMaintenance, createDatabase, databaseExists, dropDatabase, readMeta, templateExists, touch, writeMeta, } from "./db.js";
 ...
 */
export async function openServer(config, log) {
    try {
        ...
        if (!config.ensure) {
            throw new ProvisionError(`cannot reach ${new URL(config.server).host}: ${reason}. Is the server running?`);
        }
        ...
        throw new ProvisionError(`cannot reach ${new URL(config.server).host} after running "${config.ensure}"`);
    }
}
/**
 ...
 */
export async function provision(client, config, database, branch, log) {
    ...
    if (await databaseExists(client, database)) {
        ...
        if (config.setup?.on === "always")
            runSetup(config, env, log);
        return { database, url, created: false };
    }
    const template = config.template && (await templateExists(client, config.template))
        ? config.template
        : undefined;
...
export async function reset(client, config, database, branch, log) {
 ...
 */
function setupEnv(config, url) {
    return { ...childEnv(config, url), DATABASE_URL: url };
}
function runSetup(config, env, log) {
...
export function childEnv(config, url) {
    const exported = { DATALESS_URL: url };
    for (const name of config.exportAs)
        exported[name] = url;
