 ...
 * anything.
 */
export function currentBranch(cwd, env) {
    const override = env["DATALESS_BRANCH"];
    if (override)
        return override;
    const branch = git(cwd, ["rev-parse", "--abbrev-ref", "HEAD"]);
    if (!branch) {
        throw new BranchError(`${cwd} is not a git repository (set DATALESS_BRANCH to name the database yourself)`);
    }
...
export function isDetached(branch) {
 ...
 */
export function databaseName(prefix, branch) {
    const slug = slugify(branch);
    const full = `${prefix}_${slug}`;
    if (Buffer.byteLength(full) <= MAX_IDENTIFIER_BYTES)
        return full;
    ...
    if (room <= 0) {
        throw new BranchError(`"${prefix}" leaves no room for a branch name within ${MAX_IDENTIFIER_BYTES} bytes`);
    }
