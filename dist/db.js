 ...
 */
export async function connectMaintenance(server) {
...
export async function databaseExists(client, name) {
    const { rows } = await client.query("select 1 as one from pg_database where datname = $1", [name]);
...
export async function createDatabase(client, name, template) {
    ...
    try {
        ...
        if (code(err) === DUPLICATE_DATABASE)
            return false;
        if (template && err instanceof Error && /being accessed by other users/.test(err.message)) {
            throw new DatabaseError(`cannot copy "${template}": something is connected to it. ` +
                `A template has to be a database nothing runs against.`);
        }
        throw err;
    }
}
export async function templateExists(client, template) {
    return await databaseExists(client, template);
}
/** COMMENT takes a literal, not a parameter, so the JSON is quoted by hand. */
export async function writeMeta(client, name, meta) {
    await client.query(`COMMENT ON DATABASE ${quoteIdent(name)} IS ${quoteLiteral(JSON.stringify(meta))}`);
}
export async function readMeta(client, name) {
    const { rows } = await client.query(`select shobj_description(oid, 'pg_database') as comment from pg_database where datname = $1`, [name]);
    return parseMeta(rows[0]?.comment ?? null);
}
export async function touch(client, name) {
 ...
 */
export async function listManaged(client, prefix) {
    const { rows } = await client.query(`select d.datname,
    ...
    for (const row of rows) {
        ...
        if (!meta)
            continue;
        if (prefix && !row.datname.startsWith(`${prefix}_`))
            continue;
        entries.push({
...
export async function terminate(client, name) {
    const { rows } = await client.query(`select count(*)::text as closed
       from (select pg_terminate_backend(pid)
               from pg_stat_activity
              where datname = $1 and pid <> pg_backend_pid()) t`, [name]);
 ...
 */
export async function dropDatabase(client, name, force = false) {
    if (force)
        await terminate(client, name);
    try {
        await client.query(`DROP DATABASE ${quoteIdent(name)}`);
    }
    catch (err) {
        if (code(err) === UNDEFINED_DATABASE)
            return;
        if (!force && err instanceof Error && /being accessed by other users/.test(err.message)) {
            throw new DatabaseError(`"${name}" is in use. Stop what is connected, or pass --force to close it.`);
        }
        throw err;
    }
}
